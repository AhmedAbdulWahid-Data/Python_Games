...
class SurfaceMaker:
	def __init__(self):
		...
		for index, info in enumerate(walk('/Users/ahmedabdulwahid/Desktop/Breakout/graphics/blocks')):
			if index == 0:
				self.assets = {color:{} for color in info[1]}
			else:
				for image_name in info[2]:
					color_type = list(self.assets.keys())[index - 1]
					full_path = '/Users/ahmedabdulwahid/Desktop/Breakout/graphics/blocks' + f'/{color_type}/' + image_name
